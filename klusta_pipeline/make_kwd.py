...
def get_args():
    ...
    parser.add_argument(
        'rig',
        type=str,
        help='defines the rig that the data was collected on. Options include: %s' %
        (str(
            list(
                port_site.keys()))))
    parser.add_argument(
    ...
    # TODO: change probe.py so that import and automatic update of the
    ...
    parser.add_argument(
        ...
        type=str,
        default='spline',
        help='sets realignment method. Options include: %s' %
        (str(
            list(
                realign_methods.keys()))))
    parser.add_argument(
        '--upper',
        dest='upper_thresh',
        type=float,
        default=4.5,
        help='Sets the upper threshold in std for spike detektion')
    parser.add_argument(
        '--lower',
        dest='lower_thresh',
        type=float,
        default=2,
        help='Sets the lower threshold in std for spike detektion')
    parser.add_argument(
        '--prespike',
        dest='prespike',
        type=float,
        default=0.8,
        help='Sets the time in milliseconds to take prior to spike peak')
    parser.add_argument(
        '--postspike',
        dest='postspike',
        type=float,
        default=1.5,
        help='Sets the time in milliseconds to take after spike peak')
...
def main():
    ...
    if os.path.exists(kwd):
        raise IOError(
            '%s already exists, please delete or rename and run again' %
            kwd)
    ...
    if args.fs is None:
        interval = None
        for rec in mat_data:
            assert interval is None or interval == rec['interval'][
                0], "intervals don't match between all the recordings... something seems wrong"
    ...
    for import_file in import_list:
        recordings = load_recordings(
            import_file, chans, inc_times=(
                args.realignment is 'spline'))
        for r in recordings:
