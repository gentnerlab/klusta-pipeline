...
def load_stim_info(s2mat):
    try:
        with h5.File(s2mat, 'r') as f:
            ...
            codes = np.array(
                [c for c in f['stimulus_textmark']['codes'][0, :]])
            names = np.array([get_textmark(x)
                              for x in np.transpose(f['stimulus_textmark']['text'])])
    except KeyError:
        with h5.File(s2mat, 'r') as f:
            times = np.array(f['stimulus_']['times']).T.squeeze()
            codes = np.array([c for c in f['stimulus_']['codes'][0, :]])
            names = np.array([get_textmark(x)
                              for x in np.transpose(f['stimulus_']['text'])])
