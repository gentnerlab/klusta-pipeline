        ...
        with h5.File(s2mat, 'r') as f:
            for ch in chans:
                try:
                    chan_data = f[ch]
                    mat_chans.append(ch)
                    assert interval is None or interval == chan_data['interval'][
                        0], "intervals don't match between channels in %s ... something seems wrong" % (s2mat)
...
def get_file_info(filename):
    ...
    if filename.startswith('AutoSv'):
        d.update(datetime=dt.datetime.strptime(
            filename[7:22], '%m%d%y_%H-%M-%S').ctime())
    elif filename.startswith('Sub'):
        print(filename)
        datestr = filename.split('_')[1]
        d.update(datetime=dt.datetime.strptime(
            datestr, '%m-%d-%y+%H-%M-%S').ctime(), )
    else:
        pass
...
def do_car(data):
    ...
    for ch, waveform in enumerate(data.T):
        common_av = np.vstack(
            (data.T[:ch, :], data.T[ch + 1:, :])).mean(axis=0)
...
def subsample_data(data, npts=1000000, axis=0):
    pts = data.shape[0]
    indx = sample(range(pts), npts) if pts > npts else list(range(pts))
    return data[indx, :]
...
def do_inplace_chunked_war(data, weights, gigs_chunksize=.5):
    ...
    chunks = np.linspace(
        0, data.shape[0], int(
            data.nbytes * gigs_chunksize / 1024 / 1024 / 1024) + 1, dtype=int)
