...
def get_channel_groups(probe, s):
    ...
    # probes A1x32-Edge-xmm-20s-yyy
        ...
        for i in range(4):
            ...
            channel_groups[i]['channels'] = list(
                channel_groups[i]['geometry'].keys())
        ...
        channel_groups = {0: {'channels': list(s.values()), 'geometry': {s[i]: (
            0, idx * 50) for idx, i in enumerate(itertools.chain(*list(zip(list(range(1, 17)), list(range(32, 16, -1))))))}, }}
    else:
...
def get_graph_from_geometry(geometry):
    ...
    chans, coords = list(zip(*[(ch, xy) for ch, xy in geometry.items()]))
...
def clean_dead_channels(channel_groups):
    ...
    for gr, group in channel_groups.items():
        ...
        new_group[gr]['channels'] = [
            ch for ch in group['channels'] if ch is not None]
        new_group[gr]['geometry'] = {ch: xy for (
            ch, xy) in group['geometry'].items() if ch is not None}
...
def load_probe(filename):
    prb = {}
    exec(compile(open(filename).read(), filename, 'exec'), {}, prb)
...
def plot_channel_groups(channel_groups):
    ...
    for sh in range(n_shanks):
        ...
        for pr in channel_groups[sh]['graph']:
            ...
            ax[sh, 0].plot(*list(zip(*points)), color='k', alpha=0.2)
...
def _get_args():
    ...
    parser = argparse.ArgumentParser(
        description='gdisplay the geometry defined in a probe file')
